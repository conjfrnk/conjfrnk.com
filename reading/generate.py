...
# Write the lists to lists.html (kept for backwards compatibility)
with open(output_html_file, "w", encoding="utf-8") as f:
    f.write(lists_content)

print(f"Generated book lists in {output_html_file}")

# Also inline the lists into ../books.html between BOOKS:START/BOOKS:END markers
# so the content is pre-rendered for SEO and crawlers without JS.
import re
...
if os.path.exists(books_html_path):
    ...
    if count:
        with open(books_html_path, "w", encoding="utf-8") as f:
            f.write(updated)
        print(f"Inlined book lists into {books_html_path}")
    else:
        print(f"Warning: BOOKS:START/BOOKS:END markers not found in {books_html_path}")
